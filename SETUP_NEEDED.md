...
- [ ] Create a GCP project (or confirm an existing one)
- [ ] Enable billing on the project
- [ ] Note the **Project ID** (not display name)
...
```hcl
project_id = "your-actual-project-id"   # ← Replace CHANGE_ME
```

Edit `infra/production.tfvars` when ready:
```hcl
...
This provisions: VPC networking, Cloud SQL (Postgres 16), Memorystore (Redis), Secret Manager secrets, Artifact Registry, Cloud Tasks queue, Cloud Run service (placeholder image), Workload Identity Federation for GitHub Actions.
