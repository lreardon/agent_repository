...
LISTING_PRICE = os.environ.get("DEMO_LISTING_PRICE", "0.005")
...
def main() -> None:
    ...
    step(4, "Alice funds her account (needed for storage fees on delivery)")
    alice_addr = expect(alice.get(f"/agents/{alice.agent_id}/wallet/deposit-address", signed=True), 200, "Alice deposit address")
    agent_says("Alice", BLUE, f"Deposit address: {alice_addr['address']}")
    ...
    step(7, "Bob proposes a job with script-based acceptance criteria")
    ...
    step(10, "Alice accepts Bob's terms")
    acceptance_criteria = {
        "version": "2.0",
        "script": script_b64,
        "runtime": "python:3.13",
    ...
    step(11, "Bob funds escrow — credits locked by the platform")
...
""")
    ...
    step(14, "Platform runs Bob's verification script in sandbox")
    t0 = time.time()
    ...
    bob_spent = Decimal(DEPOSIT_AMOUNT) - bob_remaining
    ...
    step(16, "Alice checks her wallet balance")
    ...
    agent_says("Alice", BLUE, f"Pending withdrawals: ${wallet_bal['pending_withdrawals']}")
    ...
    platform_says(f"Withdrawal {data['status']} → USDC will be sent to {data['destination_address'][:10]}...")
    show_json(data, ["withdrawal_id", "amount", "fee", "net_payout", "status"])

    step(18, "Alice's balance after withdrawal")
    alice_bal_after = expect(alice.get(f"/agents/{alice.agent_id}/balance", signed=True), 200, "Alice balance")
    agent_says("Alice", BLUE, f"Remaining balance: ${alice_bal_after['balance']}")
    platform_says("Balance deducted immediately — USDC payout processed in background")

    step(19, "Alice checks her transaction history")
    ...
    if txns["withdrawals"]:
        show_json(txns["withdrawals"][0], ["amount", "fee", "net_payout", "status", "destination_address"])

    # ═══════════════════════════════════════════════════════════════
    banner("Act 9: Reputation — Agents Review Each Other")
    # ═══════════════════════════════════════════════════════════════

    step(20, "Bob reviews Alice (client → seller)")
    ...
    step(21, "Alice reviews Bob (seller → client)")
    ...
    step(22, "Alice's reputation on the platform")
